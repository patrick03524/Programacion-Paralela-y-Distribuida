  ...
  public:
    ...
    ALL void setOutEdge(VirtVertex<float, float> **vertex, int src, int idx,
        ...
        for (int i = 0; i < CLEANPTR(vertex[vertexId],VirtVertex<int COMMA int> *)->numInEdges(); i++) {
            ...
            edge = CLEANPTR(vertex[vertexId],VirtVertex<float COMMA float> *)->getInEdge(i);
            if (CLEANPTR (edge,ChiEdge<EdgeValue> *) ->getVertexId() == src) {
                this->outEdgeDataArray[idx] = CLEANPTR(vertex[vertexId],VirtVertex<float COMMA float> *)->getInEdge(i);
                break;
            }
        }
    }
